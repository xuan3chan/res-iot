...
async function run() {
  ...
  try {
    ...
    let user = usersRes.data.find(
      (u) => u.name === 'Face User' || u.name === 'Direct Test' || u.email.includes('faceuser')
    );
    let testEmail = '';
    ...
    if (!user) {
      ...
      try {
        ...
        console.error(
          ...
          createErr.response?.data || createErr.message || createErr
        );
      }
    ...
    try {
      const registerRes = await axios.post(`${GATEWAY_URL}/users/${user.id}/register-face`, form, {
    ...
    try {
      const verifyRes = await axios.post(`${GATEWAY_URL}/users/verify-face`, verifyForm, {
